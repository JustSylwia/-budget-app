...
var state = {
  nextId: 3,
  incomings: [
    { id: 1, name: "Wypłata", amount: 3000, isEditable: false },
    { id: 2, name: "Ekstra Zlecenie ", amount: 1000, isEditable: false },
  ],
  nextId2: 3,
  outgoings: [
    { id: 1, name: "Czynsz", amount: 600, isEditable: false },
    { id: 2, name: "Jedzenie", amount: 1200, isEditable: false },
  ],
};
...
function confirmEditItem(e) {
  ...
  var spans = document.querySelectorAll(
    `div.item.item-incoming [data-id="${id}"]`
  );
...
function confirmEditItem2(e) {
  var id = Number(e.target.dataset.id);
  var spans = document.querySelectorAll(
    `div.item.item-outgoing [data-id="${id}"]`
  );
  var newName = spans[0].innerText;
  var newAmount = Number(spans[1].innerText);
  state.outgoings = state.outgoings.map((i) =>
